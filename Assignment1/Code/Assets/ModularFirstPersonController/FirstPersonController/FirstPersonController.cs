...
public class FirstPersonController : NetworkBehaviour
{
    ...
    #region Action
    public enum Action {None, ThrowAxe, ChopTree, AddWood}
    ...
    private void OnTriggerEnter(Collider other) {
        if (other.name == "ThrowArea") {
            crosshairObject.gameObject.SetActive(!crosshairObject.IsActive());
            action = Action.ThrowAxe;
        } else if (other.CompareTag("treeTrigger")){
            crosshairObject.gameObject.SetActive(!crosshairObject.IsActive());
            action = Action.ChopTree;
        } else if (other.CompareTag("FireTrigger")) {
            crosshairObject.gameObject.SetActive(!crosshairObject.IsActive());
            action = Action.AddWood;
        }
    }

    private void OnTriggerExit(Collider other) {
        if (other.name == "ThrowArea" || other.CompareTag("treeTrigger") || other.CompareTag("FireTrigger")) {
            crosshairObject.gameObject.SetActive(!crosshairObject.IsActive());
            action = Action.None;
        }
    }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUI.enabled = fpc.playerCanMove;
        fpc.rotationSpeed = EditorGUILayout.Slider(new GUIContent("Rotation Speed", "Determines how fast the player will rotate."), fpc.rotationSpeed, 100f, 300.0f);
        GUI.enabled = true;
        ...
        GUI.enabled = fpc.playerCanMove;
        fpc.swimDrag = EditorGUILayout.Slider(new GUIContent("Swimming Drag", "Determines how much the player will slow in water."), fpc.swimDrag, 0f, 50f);
        GUI.enabled = true;
